...
const getModuleRoot = _.memoize(function getModuleRoot () {
  ...
  while (!isAtFsRoot) {
    ...
    try {
      if (_fs.existsSync(manifestPath) &&
          JSON.parse(_fs.readFileSync(manifestPath, 'utf8')).name === 'appium-webdriveragent-ios') {
        return currentDir;
      }
    } catch (ign) {}
  ...
  throw new Error('Cannot find the root folder of the appium-webdriveragent-ios Node.js module');
});
